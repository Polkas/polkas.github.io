...
- **Configure your hooks** in the `.pre-commit-config.yaml` file, including hooks like `lintr` or `styler`. The `precommit::use_precommit()` run creates a decent template of `.pre-commit-config.yaml` file.  
