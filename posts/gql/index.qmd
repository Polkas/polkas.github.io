---
title: "Introduction of GQL and Graph DB"
...
# Introduction of GQL and Graph DB

TL;DR: Graph databases have emerged as a crucial technology for managing and analyzing data that is highly interconnected. Unlike traditional relational databases that store data in tables, graph databases use a graph structure to represent and store data, making them exceptionally powerful for applications that involve complex relationships and connections. Graph databases have reached a significant milestone with the publication of the Graph Query Language (GQL) as an ISO standard. This development marks a new era in the standardization and usability of graph databases, providing a unified query language that enhances the way we interact with graph data. It is the first database query language to be certified since SQL in 1987.

[Please access the ready Graph DB playground](https://github.com/Polkas/gql-workflow-demo)
...
## Example: Full GQL Workflow

This example demonstrates a comprehensive workflow for working with Graph Query Language (GQL) in a graph database. It includes defining a schema, inserting data, querying data, performing advanced queries, and running graph algorithms. 

[Please access the ready playground](https://github.com/Polkas/gql-workflow-demo)

## Where to Run These Queries

To execute the GQL commands provided, you need a graph database system that supports GQL or similar query languages. Follow these steps to get started:

- **Local Setup**:
   - Install Neo4j, TigerGraph, or another graph database system.
   - Use their query editor or command-line tools to define schemas, insert data, and run queries.

- **Cloud-Based Platforms**:
   - Sign up for services like [Neo4j Aura](https://neo4j.com/cloud/aura/) or [TigerGraph Cloud](https://www.tigergraph.com/cloud/).
   - Deploy a database instance and run your GQL commands in the browser-based query editor.
...
- [Graph DB playground](https://github.com/Polkas/gql-workflow-demo)
